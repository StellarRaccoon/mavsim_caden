...
%--------------------------------
    ...
    methods
        ...
        self.Va
            ...
            % Input vector generated by thrust vectoring
            R_tv = [cos(tv1)*cos(tv2); cos(tv1)*sin(tv2); sin(tv1)*cos(tv2)];
            %% PROPELLER FORCE AND TORQUE Algorithim 4.1
            [F_prop, Q_prop, Omega_prop] = prop_force_and_torque(self.Va, delta_t,MAV);
            % below is a simpliefied propeller force 
            % F_prop = 0.5*MAV.rho*MAV.S_prop*MAV.C_prop*((MAV.k_motor*delta_t)^2-self.Va^2);
            % Rotate Q_prop, its like M_drag, to counteract in the direction the prop faces
            M_drag = -Q_prop*R_tv;
            F_prop = F_prop*R_tv;
            ...
            %% AERODYNAMIC MOMENTS
                ...
                %get the derivative of H_prop
                tv1_dot = (tv1 - self.prev_tv1)/self.ts_simulation;
                tv2_dot = (tv2 - self.prev_tv2)/self.ts_simulation;
                Omega_prop_dot = (Omega_prop - self.prev_Omega_prop)/self.ts_simulation;
                H_prop = MAV.I_s*Omega_prop*R_tv; % Kikkawa eq7
                H_prop_dot = [...
                            MAV.I_s*Omega_prop_dot*cos(tv1)*cos(tv2) - MAV.I_s*Omega_prop*tv1_dot*sin(tv1)*cos(tv2) - MAV.I_s*Omega_prop*tv2_dot*cos(tv1)*sin(tv2);
                            MAV.I_s*Omega_prop_dot*cos(tv1)*sin(tv2) - MAV.I_s*Omega_prop*tv1_dot*sin(tv1)*sin(tv2) + MAV.I_s*Omega_prop*tv2_dot*cos(tv1)*sin(tv2);
                            MAV.I_s*Omega_prop_dot*cos(tv1)*sin(tv2) + MAV.I_s*Omega_prop*tv1_dot*sin(tv1)*cos(tv2) - MAV.I_s*Omega_prop*tv2_dot*sin(tv1)*cos(tv2)
                            ];  
                
                M_prop = -(H_prop_dot+cross([p;q;r],H_prop)); %Kikkawa eq8

                %moment generated by anfles of the thrust vectoring mehchanism
                M_servo = Veh2Body(phi,theta,psi)'*([-MAV.I_s*Omega_prop_dot; -MAV.I_s*Omega_prop*tv1_dot; MAV.I_s*Omega_prop*tv2_dot]);
                
                Torque = 1/2*MAV.rho*self.Va^2*MAV.S_wing*M_aero+1/2*MAV.rho*self.Va^2*MAV.S_wing*M_cs+M_prop+M_drag+M_servo;
                % sum forces together as in eq 4.24
                % Torque = 1/2*MAV.rho*self.Va^2*MAV.S_wing*M_aero+1/2*MAV.rho*self.Va^2*MAV.S_wing*M_cs+[-Q_prop;0;0];
                self.prev_Omega_prop = Omega_prop;
                self.prev_tv1 = tv1;
                self.prev_tv2 = tv2;
        ...
        function self=update_true_state(self, wind)
            ...
            % self.true_state.Omega_prop = self.Omega_prop; %update to the newest omega prop
        end
    function reset_tv_history(self, tv1, tv2, Omega_prop)
    self.prev_tv1 = tv1;
    self.prev_tv2 = tv2;
    self.prev_Omega_prop = Omega_prop;
    end

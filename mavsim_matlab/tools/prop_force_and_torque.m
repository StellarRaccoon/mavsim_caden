...
function [T_prop, Q_prop] = prop_force_and_torque(Va, delta_t, MAV)
    ...
    % r = roots([a, b, c]);
        ...
        r = r(imag(r)==0 & r > 0);
        if isempty(r)
            Omega_prop = 1e-3;   % fallback to tiny but nonzero RPM
        else
            Omega_prop = max(r);
        end

            % Omega_prop = max(r(r > 0))
            % Omega_prop = max(Omega_prop, 1e-3); %if omega_prop basically 0, contrain it so its not too small
